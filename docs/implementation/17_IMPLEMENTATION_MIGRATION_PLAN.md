...
- **Tests:** `c5-adversarial.test.ts` (16), `luna-generation-benchmark.test.ts` rewrite (17 — HC01–HC10 expectations, HC05→HC06 trace transition, coverage matrix, readiness true/false, exact-copy hard gate, machine-readable report). **489 pass / 5 skipped.**
- **COVERAGE PATCH (2026-09-02):** HC05/HC06 now carry the `selectLearningTargets` trace — HC05 rejects `M7.ALG.SYMMETRIC` (origin 9) with reason naming the `M7.ALG.IDENTITY` bridge; HC06 (bridge + Grade-8 algebra mastered) selects SYMMETRIC as a FRONTIER target at origin 9 with `prerequisiteRepair: 0` (`selectionReason` MASTERED_FRONTIER_STRETCH — the child carried Grade-9 evidence, an explicit reason). New **HC09** (true ESTIMATED / Curriculum-Clock-only context) and **HC10** (legitimate K5 from strong verified frontier, T independently determined ≤ T4). `assessBenchmarkReadiness` criteria widened (grade4+7 / FRONTIER / K4 / K5 / T4 / T5 / grade-level T4T5 / Parallel Gap Repair / HC05→HC06 progression / no-frontier HSG / ESTIMATED context / SUPPORTING+STRONG+VERIFIED all represented). **`benchmarkReady: true`, blocking gaps: none** (16 base + 10 hard). Non-blocking (visible): `kha_gioi` goal + `choice`/`exact` answer format + deterministic-correctness item (mock-only).
