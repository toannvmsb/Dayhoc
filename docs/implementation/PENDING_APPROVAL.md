...
| **OD-4 `authorize()` route cutover** | **CLOSED — every production Child-data route.** `createProductionApi` covers identity/workspace, parent/child, enrollment, learning (context / today / **parentHome / parentProgress / parentGapDetail**), school/class, relationships, discovery, teacher, student, and the practice loop (assignment / attempt / evidence). All server-derived identity + `authorize()` / `can()`. `createApi` (legacy) is fail-closed in production and consumed by nothing — kept for unit tests + the internal pre-authorized projection delegation (F-dev §0c). | delete `createApi`'s learning methods once nothing delegates to them (they are the tested pipeline `_scene` exposes). |
