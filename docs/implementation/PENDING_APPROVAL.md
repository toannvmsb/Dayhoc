...
|---|---|---|
| P-01 | **Em tự rà skill graph/DAG lớp 7 theo SGK trước → xuất bản đề xuất, anh duyệt.** Chưa freeze `M7.*`. | ✅ **Bản đề xuất xong (2026-09-01, anh chốt Q4 = em làm ngay):** `docs/implementation/M7_SKILL_REVIEW.md` + `.csv` (import Google Sheets) + `packages/math-data/data/m7-dag-additions.yaml` (11 cạnh DAG đề xuất, chưa nạp). Kết quả: độ phủ 37/37 bài SGK đúng 1:1 ✅; đề xuất đổi tên `M7.RAT.*`→`M7.QNUM.*` (dễ nhầm `RATIO`); thiếu 11 cạnh prereq giữa chương (chủ yếu Chương IV↔IX tam giác + Chương X hình khối). **Chờ anh duyệt 5 mục ở §5** rồi em nạp + freeze ID. |
