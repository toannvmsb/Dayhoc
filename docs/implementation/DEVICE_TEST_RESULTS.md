...
| 5 | Practice (create → runner → submit → "Xong rồi!") | ✅ | Retest after fixes: runner opens, per-question ✓/✗, "Đáp án đúng" + input format hint all show. First pass found **D-06 P2** (no result) + **D-07 P1** (cold-server login crash) — both fixed & confirmed on device — and **D-08 P3** (`exact` answers need exact phrasing — mitigated by revealing the answer). Enhancement per tester: completion screen also shows "Con trả lời: …" next to "Đáp án đúng: …" for comparison (`a8c559c`). | P1 / P2 / P3 (fixed) | `bf13fad` / `a8c559c` |
...
| D-09 | P2 | 6 | Mobile parent app has no screen to create / view / revoke a child's student login — the `/children/:id/student-access` routes exist and the web `/be/[childId]/ho-so` has the UI, but mobile Settings does not. Device Test 6 "create login" can only be done from web or the seed | open — small parity add to `apps/mobile/app/parent/settings.tsx` (backend ready). Test 6 proceeds with the seeded student account meanwhile |
