...
| Seed | **Re-seeded clean 2026-09-03** (DB had ~13 duplicate `@dayzi.seed`/`@dayzi.local` users from repeated integration-test + partial-seed runs → non-deterministic dev-auth login; manually purged, then `seed:pilot`). Now: parent `phuhuynh@dayzi.seed`/`pilotpass1234`, teacher `giaovien@dayzi.seed`/`pilotpass1234` (Cô Pilot), student `hs-8cd01077@dayzi.local`/`hocsinh1234`, children *Bé Lớp 4* `84f98dd0-7d79-4f13-93f7-fce3bebfe9c5` (has evidence/gap) / *Bé Lớp 7* `8cd01077-3ba6-4664-be82-f007342c5451`. Verified: no dup emails, parent→2 children, student roles `[STUDENT]`, teacher roles `[TEACHER]`. |
...
| 7 | Student practice (self-luyện → submit) | 🔄 | Tapping a "Hôm nay" task ("Ôn tập") crashed: `invalid input for type uuid: asg_1` — the today tasks carry ephemeral plan ids (`asg_1`…), not real assignment rows → **D-11 P1, fixed**. Also **D-11b P3**: raw PG error leaked to the client → uuid guard now returns a friendly 404. Retest pending | P1 / P3 | `<pending>` |
