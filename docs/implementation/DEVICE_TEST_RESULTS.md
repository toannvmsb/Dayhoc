...
| Seed | **Re-seeded clean 2026-09-03** (DB had ~13 duplicate `@dayzi.seed`/`@dayzi.local` users from repeated integration-test + partial-seed runs → non-deterministic dev-auth login; manually purged, then `seed:pilot`). Re-created again 2026-09-03 after the D-12 role fix. Now: parent `phuhuynh@dayzi.seed`/`pilotpass1234`, teacher `giaovien@dayzi.seed`/`pilotpass1234` (Cô Pilot), student `hs-dea04d6e@dayzi.local`/`hocsinh1234` (Bé Lớp 7), children *Bé Lớp 4* `a6b5f5f2-e9a3-4533-a48b-e8bbef5c08fd` (has evidence/gap) / *Bé Lớp 7* `dea04d6e-e179-43da-9f3d-79b133e899af`. Verified: no dup emails, parent→2 children, `users.role` = parent/teacher/child respectively, `user_roles` = `[PARENT]` / `[TEACHER]` / `[STUDENT]`. Bé Lớp 4 has **no** student login yet (for testing "create"). |
...
| 7 | Student practice (self-luyện → submit) | 🔄 | (1) Tapping a "Hôm nay" task crashed on `asg_1` → **D-11 P1 fixed** + **D-11b P3** (raw PG error → friendly 404). (2) Logging in as the student landed on the parent "Thêm con của bạn" screen — student accounts were wrongly getting a PARENT role → **D-12 P1 fixed** (`insertUser` wrote legacy `users.role='parent'` for everyone incl. students; the legacy→identity backfill — run by the shared-DB integration tests — then granted PARENT). Retest pending | P1 / P3 | `7f760fb` / `<pending>` |
