...
| 12 | School / Class (propose school, "Thêm lớp", gán con, privacy) | 🔄 | Mobile had **no** school/class UI → **D-20 P2 fixed** (`app/parent/school.tsx` + 8 rest.ts routes). Full flow verified over LAN: search "Thử Nghiệm" → enroll school → propose class "4A9" → assign child PRIMARY/LINKED_PRIVATE → PATCH privacy → LINKED_SHARED. `listEnrollments` now returns schoolName/className. Bé Lớp 4 enrollments cleaned for a fresh run. Ready to test. | P2 | `efbea57` |
