...
| 10 | Parent ↔ Teacher connection (invite code → approve) | 🔄 | Mobile had **no** parent connection UI → **D-18 P2 fixed** (`app/parent/connect.tsx`, linked from Settings). Full flow verified over LAN: code → redeem → PENDING → accept → teacher sees Bé Lớp 4. Bé Lớp 4 teacher link cleaned so the tester runs it fresh. Ready to test. | P2 | `3e1e70a` |
| 11 | Permission grant / revoke (sensitive toggle reflects for teacher) | 🔄 | Same screen (D-18). Verified over LAN: before grant teacher `/twin` → "no ACTIVE grant" denied; after parent toggles VIEW_LEARNING_TWIN_SUMMARY the teacher twin returns `M4.FRAC.COMMON_DENOM: cần_củng_cố`. Ready to test. | — | `3e1e70a` |
