...
| 7 | Student practice (self-luyện → submit) | 🔄 | Retest after D-11/D-12: task opens, practice runs & submits, result screen OK. New findings: (3) after finishing the day's work the plan → `no_plan_needed` and "Hôm nay" showed a bare "không có bài bắt buộc. Nghỉ ngơi nhé!" with the earlier "Thử thách" gone → **D-14 P3** — now shows the plan's own reason + a "Luyện thêm 15 phút" button. (4) opening "Tiến bộ" right after submit flashed an error (self-healed on refresh) — concurrent `refreshLearningState` cache-write race, HTTP 409 `23505` → **D-13 P2 fixed**. Retest pending | P1/P2/P3 | `7f760fb` `8019efb` `9596b8d` |
