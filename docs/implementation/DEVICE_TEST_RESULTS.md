...
| DB | portable PG 16, **`parent_copilot_pilot`** — a dedicated DB for device testing (2026-09-03), 20 migrations. The test suite still uses `parent_copilot`; `pg-store.integration.test.ts` runs the legacy→identity backfill on whatever DB `DATABASE_URL` points at, so a `vitest run` was mutating the device-test seed. Launch the API with `DATABASE_URL=postgres://postgres@127.0.0.1:5432/parent_copilot_pilot` (overrides `.env.local`). |
| Seed | **Re-seeded clean 2026-09-03** (DB had ~13 duplicate `@dayzi.seed`/`@dayzi.local` users from repeated integration-test + partial-seed runs → non-deterministic dev-auth login; manually purged, then `seed:pilot`). Seeded into `parent_copilot_pilot` 2026-09-03: parent `phuhuynh@dayzi.seed`/`pilotpass1234`, teacher `giaovien@dayzi.seed`/`pilotpass1234` (Cô Pilot), seed student `hs-5d723a0a@dayzi.local`/`hocsinh1234` (Bé Lớp 7), children *Bé Lớp 4* `d42d3461-a845-4ce0-bb57-935603cb5f7a` (has evidence/gap) / *Bé Lớp 7* `5d723a0a-8e67-4d32-9a17-1f41582f531f`. During this session a student login `hs-d42d3461@dayzi.local` was also created for Bé Lớp 4 (roles verified `[STUDENT]`). |
...
| 7 | Student practice (self-luyện → submit) | 🔄 | 2nd retest: task opens, runs & submits fine. Remaining issues → all fixed: **D-13 P2** ("Tiến bộ" flashed a 409 right after submit — `refreshLearningState` race). **D-14 P3** (bare "Nghỉ ngơi nhé!" after finishing — now shows the plan reason). **D-15 P2** (did a task but "Hôm nay" still showed 2 tasks + "Đã xong 0/2"; "Bài tập" list never updated after finishing — the today tasks were a non-tracked plan preview & screens didn't refetch on focus). Now: real assignments, `useFocusEffect` reload, accurate "done X/Y", "Đã hoàn thành" list. Retest pending | P1/P2/P3 | `7f760fb` `8019efb` `9596b8d` `8f57c20` |
