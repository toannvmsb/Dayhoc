...
| 11 | Permission grant / revoke (sensitive toggle reflects for teacher) | 🔄 | Toggle works — after granting the sensitive group the teacher's "Bé Lớp 4" shows the 3 sections (Quyền / Mức độ nắm bài / Điểm cần củng cố). Feedback → fixed **D-19**: mastery + gaps were showing raw skill IDs (`M4.FRAC.COMMON_DENOM`); server DTO now includes `skillName` ("Quy đồng mẫu số") + a Vietnamese gap-lifecycle label. Retest pending. | P2 | `3e1e70a` `0c22554` |
